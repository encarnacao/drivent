...
describe('GET /booking', () => {
  it('should return with 401 if no token is provided', async () => {
...
describe('POST /booking', () => {
  ...
  describe('when there is a valid token', () => {
    it('should return with 400 if no roomId is provided', async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const response = await server.post('/booking').set('Authorization', `Bearer ${token}`);
      expect(response.status).toBe(httpStatus.BAD_REQUEST);
    });
    it('should return with 400 if roomId is not a number', async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
    ...
    it('should return with 403 if ticket doesnt include hotel', async () => {
      const remote = false;
      const paid = true;
    ...
    it('should return with 403 if room is already full', async () => {
      const remote = false;
      const paid = true;
      const includesHotel = true;
      const { token } = await generateValidUser(remote, paid, includesHotel);
...
describe('PUT /booking', () => {
  ...
  describe('when there is a valid token', () => {
    it('should return with 400 if id is not a number', async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const response = await server.put('/booking/' + faker.lorem.word()).set('Authorization', `Bearer ${token}`);
      expect(response.status).toBe(httpStatus.BAD_REQUEST);
    });
    ...
    it('should respond with 403 if room is already full', async () => {
      const remote = false;
      const paid = true;
      const includesHotel = true;
